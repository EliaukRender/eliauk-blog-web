...
export function getPoints(shape = 'line', size, [originX, originY], pointCount, endPoints, options = {}) {
	...
	if (shape === 'line') {
		...
		for (let i = 0; i <= pointCount; i++) {
			...
			const startingPoint = __rotatePoint__([originX + i * increment, originY - pointOffset], [originX, originY], degree);
			rtn.start.push(startingPoint);

			const endingPoint = __rotatePoint__([originX + i * increment, originY + endPoints[i] - pointOffset], [originX, originY], degree);
