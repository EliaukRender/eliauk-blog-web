...
class Analyze {
	...
	constructor(player, canvas, options = {}) {
		...
		this.source = this._audioCtx.createMediaElementSource(player);
	...
	_initPlayerEvents() {
		['play', 'playing', 'seeked'].forEach((name) => {
			...
			this.player.addEventListener(name, this.start);
		});
		// ['seeking', 'waiting', 'pause', 'ended'].forEach(name => {
		//   this.player.on(name, this.stop)
		// })
		this.player.addEventListener('volumechange', this._onoVolumechange);
		this.player.addEventListener('destroy', this._onDestroy);
